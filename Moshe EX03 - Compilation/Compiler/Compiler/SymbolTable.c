...
Attributes* insert(SymobleTable* currentTable, Token* current_token) {
	if (ht_get(currentTable, current_token->lexeme) == NULL) {
		Attributes* attributes = (Attributes*)malloc(sizeof(Attributes));
		attributes->name = (char*)malloc((sizeof(char) * (strlen(current_token->lexeme)) + 1));
		strcpy(attributes->name, current_token->lexeme);

	

		printf("attributes->name = %s", attributes->name);
